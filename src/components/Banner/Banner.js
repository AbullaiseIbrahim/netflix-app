...
function Banner({ onChange }) {
  ...
  useEffect(() => {
    const getMovieInfo = async () => {
      ...
      if (selectedMovieId < movieId - 1) selectedMovieId = selectedMovieId + 1;
      else selectedMovieId = 0;
    ...
    >
      <div className="banner-content">
        ...
        <div className="button-col">
          ...
          <button
            onClick={() => handleWishlistMovie(movie.id)}
            className="button"
          >
            <i className="fa-solid fa-plus"></i>{" "}
            {isClicked ? "Added to Wishlist" : "My List"}
          </button>
        </div>
